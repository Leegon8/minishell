...
int handle_heredoc(t_msh *msh, char *delimiter)
{
    ...
    if (!content)
        return (FALSE);
    temp_file_path = "/tmp/heredoc_temp"; // Cambia esto según tus necesidades
...
int redirect_input_output(t_msh *msh)
{
    ...
    // Aquí puedes agregar más lógica para redirigir la salida estándar si es necesario

    return (TRUE);
}

// Asegúrate de llamar a redirect_input_output() antes de ejecutar el comando
